...
public class StackedLineChart extends Region {
    ...
    public StackedLineChart() {
        ...
        mouseHandler = e -> {
            ...
            if (MouseEvent.MOUSE_PRESSED.equals(type)) {
                double x = e.getX();
                double y = e.getY();
                for (ButtonShape buttonShape : buttonShapes) {
                    if (buttonShape.shape().contains(x, y)) {
                        StackedLineChart.this.daysToShow = buttonShape.daysToShow();
                        StackedLineChart.this.filter();
                        StackedLineChart.this.redraw();
                    }
                };
                for (DayShape dayShape : dayShapes) {
                    if (dayShape.shape().contains(x, y)) {
                        StackedLineChart.this.selectedDays.put(dayShape.day(), !selectedDays.get(dayShape.day()));
                        StackedLineChart.this.filter();
                        StackedLineChart.this.redraw();
                    }
                };
    ...
    private void redraw() {
        ...
        TreeMap<LocalDate, List<GlucoEntry>> sortedEntryMap = new TreeMap<>(entryMap);
        for (Entry<LocalDate, List<GlucoEntry>> mapEntry : sortedEntryMap.entrySet()) {
        ...
        ctx.fillRoundRect(stepX + 20, 10, 50, 24, 10, 10);
        ctx.strokeRoundRect(stepX + 20, 10, 50, 24, 10, 10);
        ctx.setFill(darkMode ? Constants.BRIGHT_TEXT : Constants.DARK_TEXT);
        ctx.fillText("30 days", stepX + 45, 22, 50);
        buttonShapes.add(new ButtonShape(30, new Rectangle(stepX + 20, 10, 50, 24)));

        ctx.setFill(daysToShow == 14 ? Color.GRAY : darkMode ? Constants.DARK_BACKGROUND : Constants.BRIGHT_BACKGROUND);
        ctx.fillRoundRect(stepX + 80, 10, 50, 24, 10, 10);
        ctx.strokeRoundRect(stepX + 80, 10, 50, 24, 10, 10);
        ctx.setFill(darkMode ? Constants.BRIGHT_TEXT : Constants.DARK_TEXT);
        ctx.fillText("14 days", stepX + 105, 22, 50);
        buttonShapes.add(new ButtonShape(14, new Rectangle(stepX + 90, 10, 50, 24)));

        ctx.setFill(daysToShow == 7 ? Color.GRAY : darkMode ? Constants.DARK_BACKGROUND : Constants.BRIGHT_BACKGROUND);
        ctx.fillRoundRect(stepX + 140, 10, 50, 24, 10, 10);
        ctx.strokeRoundRect(stepX + 140, 10, 50, 24, 10, 10);
        ctx.setFill(darkMode ? Constants.BRIGHT_TEXT : Constants.DARK_TEXT);
        ctx.fillText("7 days", stepX + 165, 22, 50);
        buttonShapes.add(new ButtonShape(7, new Rectangle(stepX + 160, 10, 50, 24)));
